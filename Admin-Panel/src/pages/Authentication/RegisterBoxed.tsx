...
const RegisterBoxed = () => {
    const [credentials, setCredentials] = useState<Credentials>({ name: '', email: '', password: '', role: 2 });
    ...
    const submitForm = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (!credentials.name || !credentials.password || !credentials.email) {
            showMessage('All params required', 'error');
            return;
        }
        try {
            const response = await axios.post(`${import.meta.env.VITE_APP_BACKEND_URL}/teacher/signup`, {...credentials, subject_ids: selectedSubjectIds});
                ...
                <div className="panel sm:w-[480px] m-6 max-w-lg w-full">
                    ...
                    <form className="space-y-5" onSubmit={submitForm}>
                        <div>
                            ...
                            <input id="name" type="text" className="form-input" name="name" placeholder="Enter Name" onChange={handleInputChange} />
                        </div>
                        <div>
                            <label htmlFor="email">Email</label>
                            <input id="email" type="email" className="form-input" name="email" placeholder="Enter Email" onChange={handleInputChange} />
                        </div>
                        <div>
                            <label htmlFor="password">Password</label>
                            <input id="password" type="password" className="form-input" name="password" placeholder="Enter Password" onChange={handleInputChange} />
                        </div>
